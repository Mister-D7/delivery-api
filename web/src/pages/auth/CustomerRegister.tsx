    ...
    <div className="min-h-screen flex items-center justify-center px-4" style={{ background: '#0a0a0a' }}>
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <h1 className="text-2xl font-extrabold" style={{ fontFamily: "'Unbounded', sans-serif" }}>{t('register.title')}</h1>
        </div>

        <form onSubmit={handleSubmit} className="surface-card p-6 space-y-4">
          <div>
            <label className="text-[10px] font-semibold mb-1 block" style={{ color: '#555' }}>{t('register.name')}</label>
            <input value={name} onChange={e => setName(e.target.value)} placeholder="Ex: Ahmed Benali" className="input-field" required />
          </div>
          <div>
            <label className="text-[10px] font-semibold mb-1 block" style={{ color: '#555' }}>{t('register.email')}</label>
          ...
          <div>
            <label className="text-[10px] font-semibold mb-1 block" style={{ color: '#555' }}>{t('register.phone')}</label>
            <input value={phone} onChange={e => setPhone(e.target.value)} placeholder="Ex: 0555 12 34 56" className="input-field" />
          </div>
          <div>
            <label className="text-[10px] font-semibold mb-1 block" style={{ color: '#555' }}>{t('register.password')}</label>
          ...
          <button type="button" onClick={handleGoogleLogin} className="w-full py-3 text-sm font-semibold rounded-xl flex items-center justify-center gap-2" style={{ background: '#fff', color: '#333', border: '1px solid #ddd' }}>
        ...
        <p className="text-center text-sm mt-6" style={{ color: '#8c8578' }}>
          ...
          <Link to="/auth/login" className="font-semibold no-underline" style={{ color: '#bfa24e' }}>{t('register.login')}</Link>
        </p>
      </motion.div>
    </div>
  );
