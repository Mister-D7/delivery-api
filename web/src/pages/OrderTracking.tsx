  ...
  const searchByToken = useCallback(async (searchToken: string) => {
    ...
    try {
      ...
      if (res.data && res.data.id) {
        ...
        setError('Commande introuvable. Vérifiez le token.');
      }
    } catch { setError('Commande introuvable. Vérifiez le token.'); }
      ...
      <div className="max-w-lg mx-auto">
        {!order ? (
          <div className="text-center">
            ...
            <h1 className="text-2xl font-extrabold mb-2" style={{ fontFamily: "'Unbounded', sans-serif" }}>Suivre votre commande</h1>
            <p className="text-sm mb-8" style={{ color: '#8c8578' }}>Entrez le token sécurisé reçu après votre commande.</p>

            {/* Show my orders if logged in */}
            {customer && myOrders.length > 0 && !loadingMyOrders && (
              <div className="mb-8 text-left">
                <p className="text-xs font-bold tracking-wide mb-3" style={{ color: '#bfa24e', fontFamily: "'IBM Plex Mono', monospace" }}>MES COMMANDES</p>
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        {o.items && o.items.length > 0 && (
                          <p className="text-[10px] mt-1 truncate" style={{ color: '#555' }}>
                            {o.items.slice(0, 3).map((i: any) => i.customName || i.product?.name || 'Produit').join(', ')}{o.items.length > 3 ? ` +${o.items.length - 3}` : ''}
                          </p>
                        )}
                      </div>
            ...
            {customer && !order && !loadingMyOrders && myOrders.length === 0 && (
              <div className="mb-8">
                ...
                <div className="surface-card p-6 text-center">
                  ...
                  <p className="text-sm" style={{ color: '#8c8578' }}>Aucune commande pour le moment.</p>
                </div>
              </div>
            )}

            <div className="flex gap-2">
              <input value={token} onChange={e => setToken(e.target.value)} placeholder="Token de commande" className="input-field flex-1" onKeyDown={e => e.key === 'Enter' && handleSearch()} />
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
            ...
            <p className="text-xs tracking-[0.25em] font-semibold mb-2" style={{ color: '#bfa24e', fontFamily: "'IBM Plex Mono', monospace" }}>SUIVI DE COMMANDE</p>
            ...
            <div className="surface-card p-5 mb-4">
              <div className="flex items-center justify-between mb-3">
                <p className="text-xs font-bold tracking-wide" style={{ color: '#8c8578' }}>DÉTAILS</p>
              ...
              <div className="space-y-2 text-sm">
                ...
                <div className="flex justify-between text-xs"><span style={{ color: '#8c8578' }}>Livraison</span><span>{order.deliveryFee != null ? (order.deliveryFee === 0 ? 'Gratuit' : `${order.deliveryFee} DA`) : '—'}</span></div>
                <div className="flex justify-between font-semibold pt-2" style={{ borderTop: '1px solid rgba(191,162,78,0.12)' }}>
                  <span>Total</span><span style={{ color: '#bfa24e' }}>{order.total} DA</span>
                </div>
              </div>
            </div>
            ...
            {order.items && order.items.length > 0 && (
              <div className="surface-card p-5 mb-4">
                <p className="text-xs font-bold tracking-wide mb-3" style={{ color: '#8c8578' }}>ARTICLES</p>
                <div className="space-y-2">
                  {order.items.map((item: any, i: number) => {
                    ...
                    return (
                      <div key={i} className="flex items-center gap-3 p-2 rounded-xl" style={{ background: i % 2 === 0 ? 'transparent' : 'rgba(255,255,255,0.02)' }}>
                        ...
                        <p className="text-sm font-bold flex-shrink-0" style={{ color: '#bfa24e' }}>{Number(item.unitPrice) * item.quantity} DA</p>
                      </div>
                    );
                  })}
                </div>
            ...
            <div className="surface-card p-5">
              ...
              <div className="space-y-2 mb-3 max-h-48 overflow-auto">
                {messages.length === 0 && <p className="text-xs text-center py-4" style={{ color: '#555' }}>Aucun message</p>}
                {messages.map(m => (
                  <div key={m.id} className="p-3 rounded-xl text-sm" style={{ background: m.sender === 'customer' ? 'rgba(191,162,78,0.08)' : '#1a1a1a', textAlign: m.sender === 'customer' ? 'right' : 'left' }}>
                    {m.text}
                    <p className="text-[10px] mt-1" style={{ color: '#555' }}>{new Date(m.createdAt).toLocaleTimeString('fr-FR', { hour: '2-digit', minute: '2-digit' })}</p>
                  </div>
                ))}
              </div>
              <div className="flex gap-2">
                <input value={newMsg} onChange={e => setNewMsg(e.target.value)} placeholder="Message..." className="input-field flex-1" onKeyDown={e => e.key === 'Enter' && sendMessage()} />
