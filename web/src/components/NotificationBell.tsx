...
import { Bell, X, Send, MessageCircle, Package, ChevronDown, ChevronUp, Volume2, Minus, CheckCheck } from '../components/adminIcons';
import api, { apiBaseURL } from '../services/api';
import { useStatusLabel } from './OrderStatusBadge';
                                    ...
                                    {order.items && order.items.length > 0 && (
                                      <div className="mt-1.5 p-2 rounded-lg" style={{ background: 'var(--admin-bg)' }}>
                                        {order.items.map((item: any, i: number) => (
                                          <div key={i} className="flex items-center gap-2 py-1" style={{ borderBottom: i < (order.items as any[]).length - 1 ? '1px solid rgba(255,255,255,0.03)' : 'none' }}>
